...
const { Client, GatewayIntentBits, Partials, MessageActionRow, MessageSelectMenu, MessageEmbed } = require("discord.js");
...
// On startup, an error will be thrown while shards are initializing.  But there is no way to detect if shards are initializing, so brute force it
const checkReady = () => {
	let limit = 10;

	client.shard.broadcastEval(c => c.ws.status).then((results) => {
		if ((results.length == client.shard.count) && (results.every(status => status === 0))) {
			setStatus();
		} else if (--limit >= 0) {
			setTimeout(checkReady, 30000);
		}
